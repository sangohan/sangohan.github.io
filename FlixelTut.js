...
ApplicationMain.preload = function() {
	...
	ApplicationMain.loadBinary("assets/data/HaxelFlixel-tutorial.oep");
	ApplicationMain.loadBinary("assets/data/room-001.oel");
	ApplicationMain.loadBinary("assets/data/room-002.oel");
	ApplicationMain.loadFile("assets/images/coin.png");
	ApplicationMain.loadFile("assets/images/enemy-0.png");
	ApplicationMain.loadFile("assets/images/enemy-1.png");
	ApplicationMain.loadFile("assets/images/health.png");
	ApplicationMain.loadFile("assets/images/player.png");
	ApplicationMain.loadFile("assets/images/tiles.png");
...
var Coin = function(X,Y) {
	...
	this.loadGraphic("assets/images/coin.png",false,8,8);
};
$hxClasses["Coin"] = Coin;
Coin.__name__ = ["Coin"];
Coin.__super__ = flixel.FlxSprite;
Coin.prototype = $extend(flixel.FlxSprite.prototype,{
	kill: function() {
		this.set_alive(false);
		flixel.tweens.FlxTween.tween(this,{ alpha : 0, y : this.y - 16},.33,{ type : 8, ease : flixel.tweens.FlxEase.circOut, complete : $bind(this,this.finishKill)});
	}
...
var DefaultAssetLibrary = function() {
	...
	var id;
	id = "assets/data/HaxelFlixel-tutorial.oep";
	this.path.set(id,id);
...
DefaultAssetLibrary.prototype = $extend(openfl.AssetLibrary.prototype,{
	...
	,getFont: function(id) {
		return js.Boot.__cast(Type.createInstance(this.className.get(id),[]) , flash.text.Font);
	}
	,getMusic: function(id) {
	...
	,loadBitmapData: function(id,handler) {
		if(this.path.exists(id)) {
			...
			loader.load(new flash.net.URLRequest(this.path.get(id)));
...
var Enemy = function(X,Y,etype) {
	...
	switch(etype) {
	case 0:
		this.loadGraphic("assets/images/enemy-0.png",true,16,16);
		break;
	case 1:
		this.loadGraphic("assets/images/enemy-1.png",true,16,16);
		break;
	default:
	...
	this._idleTmr = 0;
	this.playerPos = flixel.util.FlxPoint.get(null,null);
};
$hxClasses["Enemy"] = Enemy;
Enemy.__name__ = ["Enemy"];
Enemy.__super__ = flixel.FlxSprite;
Enemy.prototype = $extend(flixel.FlxSprite.prototype,{
	speed: null
	,etype: null
	,seesPlayer: null
	,playerPos: null
	...
	,idle: function() {
		if(this.seesPlayer) this._brain.activeState = $bind(this,this.chase); else if(this._idleTmr <= 0) {
			if(flixel.util.FlxRandom.floatRanged(0,100) < 1) {
				this._moveDir = -1;
				this.velocity.set_x(this.velocity.set_y(0));
			} else {
				this._moveDir = flixel.util.FlxRandom.intRanged(0,8) * 45;
				flixel.util.FlxAngle.rotatePoint(this.speed * .5,0,0,0,this._moveDir,this.velocity);
			}
			this._idleTmr = flixel.util.FlxRandom.intRanged(1,4);
		} else this._idleTmr -= flixel.FlxG.elapsed;
	}
	,chase: function() {
		if(!this.seesPlayer) this._brain.activeState = $bind(this,this.idle); else flixel.util.FlxVelocity.moveTowardsPoint(this,this.playerPos,this.speed | 0);
	}
	,draw: function() {
...
FSM.prototype = {
	activeState: null
	,update: function() {
		if(this.activeState != null) this.activeState();
	}
	,__class__: FSM
};
flixel.group = {};
...
var HUD = function() {
	flixel.group.FlxTypedGroup.call(this);
	this._sprBack = new flixel.FlxSprite().makeGraphic(flixel.FlxG.width,15,-16777216);
	flixel.util.FlxSpriteUtil.drawRect(this._sprBack,0,14,flixel.FlxG.width,1,-1);
	this._txtHealth = new flixel.text.FlxText(16,1,0,"3 / 3",7);
	this._txtHealth.setBorderStyle(1,-8355712,1,1);
	this._txtMoney = new flixel.text.FlxText(0,1,0,"0",7);
	this._txtMoney.setBorderStyle(1,-8355712,1,1);
	this._sprHealth = new flixel.FlxSprite(4,this._txtHealth.y + this._txtHealth.get_height() / 2 - 5,"assets/images/health.png");
	this._sprMoney = new flixel.FlxSprite(flixel.FlxG.width - 12,this._txtMoney.y + this._txtMoney.get_height() / 2 - 5,"assets/images/coin.png");
...
$hxClasses["HUD"] = HUD;
HUD.__name__ = ["HUD"];
HUD.__super__ = flixel.group.FlxTypedGroup;
HUD.prototype = $extend(flixel.group.FlxTypedGroup.prototype,{
	_sprBack: null
	,_txtHealth: null
	,_txtMoney: null
	,_sprHealth: null
...
MenuState.prototype = $extend(flixel.FlxState.prototype,{
	_btnPlay: null
	,create: function() {
		this.add(new flixel.text.FlxText(10,10,100,"Hello, World!"));
		this._btnPlay = new flixel.ui.FlxButton(0,0,"Play",$bind(this,this.clickPlay));
		flixel.util.FlxSpriteUtil.screenCenter(this._btnPlay);
	...
	,destroy: function() {
		this._btnPlay = flixel.util.FlxDestroyUtil.destroy(this._btnPlay);
...
PlayState.prototype = $extend(flixel.FlxState.prototype,{
	...
	,_health: null
	,create: function() {
		this._map = new flixel.addons.editors.ogmo.FlxOgmoLoader("assets/data/room-002.oel");
		this._mWalls = this._map.loadTilemap("assets/images/tiles.png",16,16,"walls");
		...
		this._grpEnemies = new flixel.group.FlxTypedGroup();
		this.add(this._grpEnemies);
		this._map.loadEntities($bind(this,this.placeEntities),"entities");
		flixel.FlxG.camera.follow(this._player,2,null,1);
		this._hud = new HUD();
		this.add(this._hud);
		flixel.FlxState.prototype.create.call(this);
	}
	,placeEntities: function(entityName,entityData) {
		...
		} else if(entityName == "coin") this._grpCoins.add(new Coin(x + 4,y + 4)); else if(entityName == "enemy") this._grpEnemies.add(new Enemy(x + 4,y,Std.parseInt(entityData.get("etype"))));
	}
	...
	,update: function() {
		flixel.FlxState.prototype.update.call(this);
		flixel.FlxG.overlap(this._player,this._mWalls,null,flixel.FlxObject.separate);
		flixel.FlxG.overlap(this._player,this._grpCoins,$bind(this,this.playerTouchCoin));
		flixel.FlxG.overlap(this._grpEnemies,this._mWalls,null,flixel.FlxObject.separate);
		this._grpEnemies.forEachAlive($bind(this,this.checkEnemyVision));
	}
	,checkEnemyVision: function(e) {
		if(this._mWalls.ray(e.getMidpoint(),this._player.getMidpoint())) {
			e.seesPlayer = true;
			e.playerPos.copyFrom(this._player.getMidpoint());
		} else e.seesPlayer = false;
	}
	,__class__: PlayState
});
...
Player.prototype = $extend(flixel.FlxSprite.prototype,{
	updateMovement: function() {
		...
		var _left = false;
		var _right = false;
		_up = flixel.FlxG.keys.checkKeyStatus(["UP","W"],1);
		_down = flixel.FlxG.keys.checkKeyStatus(["DOWN","S"],1);
		_left = flixel.FlxG.keys.checkKeyStatus(["LEFT","A"],1);
		_right = flixel.FlxG.keys.checkKeyStatus(["RIGHT","D"],1);
		if(_up && _down) _up = _down = false;
		if(_left && _right) _left = _right = false;
		if(_up || _down || _left || _right) {
			this.velocity.set_x(Player.speed);
			this.velocity.set_y(Player.speed);
			var mA = 0;
			if(_up) {
				mA = -90;
			...
			flixel.util.FlxAngle.rotatePoint(Player.speed,0,0,0,mA,this.velocity);
			if((this.velocity.x != 0 || this.velocity.y != 0) && this.touching == 0) {
				var _g = this.facing;
				switch(_g) {
				case 1:case 16:
...
flixel.addons.editors.ogmo.FlxOgmoLoader = function(LevelData) {
	var str = "";
	if(js.Boot.__instanceof(LevelData,Class)) str = Type.createInstance(LevelData,[]); else if(typeof(LevelData) == "string") str = openfl.Assets.getText(LevelData);
	this._xml = haxe.xml.Parser.parse(str);
	this._fastXml = new haxe.xml.Fast(this._xml.firstElement());
	this.width = Std.parseInt(this._fastXml.att.resolve("width"));
	this.height = Std.parseInt(this._fastXml.att.resolve("height"));
	flixel.FlxG.camera.setBounds(0,0,this.width,this.height,true);
};
$hxClasses["flixel.addons.editors.ogmo.FlxOgmoLoader"] = flixel.addons.editors.ogmo.FlxOgmoLoader;
flixel.addons.editors.ogmo.FlxOgmoLoader.__name__ = ["flixel","addons","editors","ogmo","FlxOgmoLoader"];
flixel.addons.editors.ogmo.FlxOgmoLoader.prototype = {
	width: null
	,height: null
	,_xml: null
	,_fastXml: null
	,loadTilemap: function(TileGraphic,TileWidth,TileHeight,TileLayer) {
		if(TileLayer == null) TileLayer = "tiles";
		if(TileHeight == null) TileHeight = 16;
		if(TileWidth == null) TileWidth = 16;
		var tileMap = new flixel.tile.FlxTilemap();
		tileMap.loadMap(this._fastXml.node.resolve(TileLayer).get_innerData(),TileGraphic,TileWidth,TileHeight);
		return tileMap;
	}
	,loadEntities: function(EntityLoadCallback,EntityLayer) {
		if(EntityLayer == null) EntityLayer = "entities";
		var actors = this._fastXml.node.resolve(EntityLayer);
		var $it0 = actors.get_elements();
		while( $it0.hasNext() ) {
			var a = $it0.next();
			EntityLoadCallback(a.get_name(),a.x);
		}
	}
	,loadRectangles: function(RectLoadCallback,RectLayer) {
		if(RectLayer == null) RectLayer = "rectangles";
		var rects = this._fastXml.node.resolve(RectLayer);
		var $it0 = rects.get_elements();
		while( $it0.hasNext() ) {
			var r = $it0.next();
			RectLoadCallback(flixel.util.FlxRect.get(Std.parseInt(r.x.get("x")),Std.parseInt(r.x.get("y")),Std.parseInt(r.x.get("w")),Std.parseInt(r.x.get("h"))));
		}
	}
	,__class__: flixel.addons.editors.ogmo.FlxOgmoLoader
};
flixel.animation = {};
...
flixel.system.FlxAssets.cacheSounds = function() {
	var _g = 0;
	var _g1 = openfl.Assets.list(openfl.AssetType.SOUND);
	while(_g < _g1.length) {
		var id = _g1[_g];
		++_g;
...
flixel.system._FlxPreloader.GraphicLogoLight.preload = function() {
	...
	flixel.system._FlxPreloader.GraphicLogoLight.image = o;
};
...
flixel.system._FlxPreloader.GraphicLogoCorners.preload = function() {
	...
	flixel.system._FlxPreloader.GraphicLogoCorners.image = o;
};
...
flixel.ui.FlxTypedButton = function(X,Y,OnClick) {
	...
	flash.Lib.get_current().get_stage().addEventListener("mouseUp",$bind(this,this.onUpEventListener));
};
...
flixel.ui.FlxTypedButton.prototype = $extend(flixel.FlxSprite.prototype,{
	...
	,destroy: function() {
		...
		flash.Lib.get_current().get_stage().removeEventListener("mouseUp",$bind(this,this.onUpEventListener));
		flixel.FlxSprite.prototype.destroy.call(this);
	}
	,update: function() {
		flixel.FlxSprite.prototype.update.call(this);
...
openfl.Assets.loadBitmapData = function(id,handler,useCache) {
	...
	if(library != null) {
		if(library.exists(symbolName,openfl.AssetType.IMAGE)) {
			...
			return;
		} else haxe.Log.trace("[openfl.Assets] There is no BitmapData asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 651, className : "openfl.Assets", methodName : "loadBitmapData"});
	} else haxe.Log.trace("[openfl.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 657, className : "openfl.Assets", methodName : "loadBitmapData"});
	handler(null);
};
openfl.Assets.loadBytes = function(id,handler) {
	...
	if(library != null) {
		if(library.exists(symbolName,openfl.AssetType.BINARY)) {
			...
			return;
		} else haxe.Log.trace("[openfl.Assets] There is no String or ByteArray asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 687, className : "openfl.Assets", methodName : "loadBytes"});
	} else haxe.Log.trace("[openfl.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 693, className : "openfl.Assets", methodName : "loadBytes"});
	handler(null);
};
openfl.Assets.loadFont = function(id,handler,useCache) {
	...
	if(library != null) {
		if(library.exists(symbolName,openfl.AssetType.FONT)) {
			...
			return;
		} else haxe.Log.trace("[openfl.Assets] There is no Font asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 744, className : "openfl.Assets", methodName : "loadFont"});
	} else haxe.Log.trace("[openfl.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 750, className : "openfl.Assets", methodName : "loadFont"});
	handler(null);
};
openfl.Assets.loadLibrary = function(name,handler) {
	...
	} else haxe.Log.trace("[openfl.Assets] There is no asset library named \"" + name + "\"",{ fileName : "Assets.hx", lineNumber : 780, className : "openfl.Assets", methodName : "loadLibrary"});
};
openfl.Assets.loadMusic = function(id,handler,useCache) {
	...
	if(library != null) {
		if(library.exists(symbolName,openfl.AssetType.MUSIC)) {
			...
			return;
		} else haxe.Log.trace("[openfl.Assets] There is no Sound asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 835, className : "openfl.Assets", methodName : "loadMusic"});
	} else haxe.Log.trace("[openfl.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 841, className : "openfl.Assets", methodName : "loadMusic"});
	handler(null);
};
openfl.Assets.loadMovieClip = function(id,handler) {
	...
	if(library != null) {
		if(library.exists(symbolName,openfl.AssetType.MOVIE_CLIP)) {
			...
			return;
		} else haxe.Log.trace("[openfl.Assets] There is no MovieClip asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 871, className : "openfl.Assets", methodName : "loadMovieClip"});
	} else haxe.Log.trace("[openfl.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 877, className : "openfl.Assets", methodName : "loadMovieClip"});
	handler(null);
};
openfl.Assets.loadSound = function(id,handler,useCache) {
	...
	if(library != null) {
		if(library.exists(symbolName,openfl.AssetType.SOUND)) {
			...
			return;
		} else haxe.Log.trace("[openfl.Assets] There is no Sound asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 934, className : "openfl.Assets", methodName : "loadSound"});
	} else haxe.Log.trace("[openfl.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 940, className : "openfl.Assets", methodName : "loadSound"});
	handler(null);
};
...
AssetPaths.HaxelFlixel_tutorial__oep = "assets/data/HaxelFlixel-tutorial.oep";
AssetPaths.room_001__oel = "assets/data/room-001.oel";
AssetPaths.room_002__oel = "assets/data/room-002.oel";
AssetPaths.coin__png = "assets/images/coin.png";
AssetPaths.enemy_0__png = "assets/images/enemy-0.png";
AssetPaths.enemy_1__png = "assets/images/enemy-1.png";
AssetPaths.health__png = "assets/images/health.png";
