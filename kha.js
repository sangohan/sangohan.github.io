...
var TurnBasedStrategyGame = function() {
	...
	this.khaRenderer = new tiled_display_KhaRenderer();
	...
	this.entityRender = new systems_RenderSystem(this.entityContainer,this.khaRenderer);
	this.mapRender = new systems_MapSystem(this.entityContainer,kha_Assets.blobs.map_tmx.toString());
	...
	this.unitSelectSystem = new systems_SelectionSystem(this.entityContainer);
...
components_MapSource.prototype = {
	...
	,isWalkable: function(x,y) {
		if(x < 0 || y < 0 || x > this.cols || y > this.rows) {
			haxe_Log.trace("isWalkable on invalid tile : " + x + ", " + y,{ fileName : "MapSource.hx", lineNumber : 49, className : "components.MapSource", methodName : "isWalkable"});
			return false;
		}
		if(x <= this.cols && y <= this.rows) {
			var tile = this.tiledMap.layers[0].tiles[x + y * this.rows];
			if(tile == null) {
				haxe_Log.trace("unexpected null tile for " + x + "," + y,{ fileName : "MapSource.hx", lineNumber : 55, className : "components.MapSource", methodName : "isWalkable"});
				return false;
			}
...
$hxClasses["de.polygonal.ds.Comparable"] = de_polygonal_ds_Comparable;
...
var de_polygonal_ds_List = function() { };
$hxClasses["de.polygonal.ds.List"] = de_polygonal_ds_List;
de_polygonal_ds_List.__name__ = ["de","polygonal","ds","List"];
de_polygonal_ds_List.__interfaces__ = [de_polygonal_ds_Collection];
var de_polygonal_ds_Dll = function(reservedSize,source) {
	...
	if(reservedSize > 0) {
		this.mHeadPool = this.mTailPool = new de_polygonal_ds_DllNode(null,this);
	}
	if(source != null && source.length > 0) {
		this.mSize = source.length;
		this.head = this.tail = this.getNode(source[0]);
		var node;
		var _g1 = 1;
		var _g = this.mSize;
		while(_g1 < _g) {
			var i = _g1++;
			node = this.getNode(source[i]);
			this.tail.next = node;
			node.prev = this.tail;
			this.tail = node;
		}
...
$hxClasses["de.polygonal.ds.Dll"] = de_polygonal_ds_Dll;
de_polygonal_ds_Dll.__name__ = ["de","polygonal","ds","Dll"];
de_polygonal_ds_Dll.__interfaces__ = [de_polygonal_ds_List];
de_polygonal_ds_Dll.prototype = {
	...
	,sort: function(cmp,useInsertionSort) {
		...
		if(this.mSize > 1) {
			...
			if(cmp == null) {
				this.head = useInsertionSort ? this.insertionSortComparable(this.head) : this.mergeSortComparable(this.head);
			} else {
				this.head = useInsertionSort ? this.insertionSort(this.head,cmp) : this.mergeSort(this.head,cmp);
			}
			if(this.isCircular) {
				this.tail.next = this.head;
				this.head.prev = this.tail;
	...
	,remove: function(val) {
		...
		var node = this.head;
		while(node != null) if(node.val == val) {
	...
	,iterator: function() {
		if(this.reuseIterator) {
			if(this.mIterator == null) {
				...
				_this.mWalker = _this.mObject.head;
				_this.mHook = null;
			}
	...
	,mergeSortComparable: function(node) {
		var h = node;
		var p;
		var q;
		var e;
	...
	,insertionSortComparable: function(node) {
		var h = node;
		var n = h.next;
		while(n != null) {
			var m = n.next;
	...
	,insertionSort: function(node,cmp) {
		var h = node;
		var n = h.next;
		while(n != null) {
			var m = n.next;
...
var de_polygonal_ds_DllIterator = function(x) {
	this.mObject = x;
	this.mWalker = this.mObject.head;
	this.mHook = null;
};
$hxClasses["de.polygonal.ds.DllIterator"] = de_polygonal_ds_DllIterator;
...
var de_polygonal_ds_CircularDllIterator = function(x) {
	this.mObject = x;
	this.mWalker = this.mObject.head;
...
$hxClasses["de.polygonal.ds.CircularDllIterator"] = de_polygonal_ds_CircularDllIterator;
...
var de_polygonal_ds_DllNode = function(val,list) {
	this.val = val;
	this.mList = list;
};
$hxClasses["de.polygonal.ds.DllNode"] = de_polygonal_ds_DllNode;
...
kha_Shaders.init = function() {
	...
	while(_g6 < 3) {
		var i6 = _g6++;
		var data6 = Reflect.field(kha_Shaders,"painter_image_vertData" + i6);
		var bytes6 = haxe_Unserializer.run(data6);
		blobs6.push(kha_internal_BytesBlob.fromBytes(bytes6));
	}
	kha_Shaders.painter_image_vert = new kha_graphics4_VertexShader(blobs6,["painter-image.vert.essl","painter-image-relaxed.vert.essl","painter-image-webgl2.vert.essl"]);
	var blobs7 = [];
	var _g7 = 0;
	while(_g7 < 3) {
		var i7 = _g7++;
	...
	while(_g8 < 3) {
		var i8 = _g8++;
		var data8 = Reflect.field(kha_Shaders,"painter_text_fragData" + i8);
		var bytes8 = haxe_Unserializer.run(data8);
		blobs8.push(kha_internal_BytesBlob.fromBytes(bytes8));
	}
	kha_Shaders.painter_text_frag = new kha_graphics4_FragmentShader(blobs8,["painter-text.frag.essl","painter-text-relaxed.frag.essl","painter-text-webgl2.frag.essl"]);
	var blobs9 = [];
	var _g9 = 0;
	while(_g9 < 3) {
		var i9 = _g9++;
...
kha_graphics2_Graphics.prototype = {
	...
	,__properties__: {set_opacity:"set_opacity",get_opacity:"get_opacity",get_fontGlyphs:"get_fontGlyphs",set_fontSize:"set_fontSize",get_fontSize:"get_fontSize",set_font:"set_font",set_color:"set_color",set_imageScaleQuality:"set_imageScaleQuality"}
};
...
systems_GuiSystem.prototype = {
	...
	,debug: function(g) {
		if(this.ui.window(zui_Handle.global.nest(1,null),this.debugGuix,this.debugGuiy,this.debugGuiWidth,this.debugGuiHeight,true)) {
...
var systems_MapSystem = function(entityContainer,xml) {
	var _g = new haxe_ds_StringMap();
	var value = { color : -16711936};
	if(__map_reserved["player"] != null) {
		_g.setReserved("player",value);
	} else {
		_g.h["player"] = value;
	}
	var value1 = { color : -65536};
	...
	while(_g1 < _g11.length) {
		var layer = _g11[_g1];
		++_g1;
		haxe_Log.trace("layer: " + layer.name,{ fileName : "MapSystem.hx", lineNumber : 26, className : "systems.MapSystem", methodName : "new"});
		var _g3 = 0;
	...
	while(_g7 < _g111.length) {
		...
		while(_g21 < _g31.length) {
			...
			var renderable = new components_Renderable(obj,(__map_reserved[entityType] != null ? _this1.getReserved(entityType) : _this1.h[entityType]).color);
			...
			case "player":
				...
				break;
			default:
				haxe_Log.trace("unknown entity type " + entityType,{ fileName : "MapSystem.hx", lineNumber : 78, className : "systems.MapSystem", methodName : "new"});
			}
		}
	}
	...
	mapSource.forEachTile(tmp1,function(x2,y2) {
		if(mapSource.isWalkable(x2,y2)) {
			...
			if(currNode == null) {
				haxe_Log.trace("no currNode: " + Std.string(currTile1),{ fileName : "MapSystem.hx", lineNumber : 97, className : "systems.MapSystem", methodName : "new"});
				return;
			}
			var _g8 = 0;
			...
			while(_g8 < _g12.length) {
				...
				if(mapSource.isWalkable(n.x,n.y)) {
					...
					if(currNeighbour == null) {
						haxe_Log.trace("no currNeighbour: " + Std.string(n),{ fileName : "MapSystem.hx", lineNumber : 104, className : "systems.MapSystem", methodName : "new"});
						return;
					}
			...
			if(guiState != null && guiState.clicked != null) {
				...
				case 0:
					...
					if(tmp1) {
						...
						haxe_Log.trace("found original entityId : " + originalEntityId[0],{ fileName : "MovementSystem.hx", lineNumber : 67, className : "systems.MovementSystem", methodName : "update"});
					...
					break;
				case 1:
					haxe_Log.trace("attack not implemented",{ fileName : "MovementSystem.hx", lineNumber : 85, className : "systems.MovementSystem", methodName : "update"});
					return;
				case 2:
					haxe_Log.trace("Cancel",{ fileName : "MovementSystem.hx", lineNumber : 88, className : "systems.MovementSystem", methodName : "update"});
					return;
				}
			}
		}
...
var systems_RenderSystem = function(entityContainer,mapRenderer) {
	this.entityContainer = entityContainer;
...
var systems_SelectionSystem = function(entityContainer) {
	this.entityContainer = entityContainer;
};
$hxClasses["systems.SelectionSystem"] = systems_SelectionSystem;
systems_SelectionSystem.__name__ = ["systems","SelectionSystem"];
systems_SelectionSystem.prototype = {
	entityContainer: null
	,update: function() {
		var mouse = this.entityContainer.getMouseEntity();
		var e = this.entityContainer.getEntities();
		while(e.hasNext()) {
			...
			if(tmp) {
				...
				if(guiState != null) {
					...
					haxe_Log.trace("selected entity: " + e1,{ fileName : "SelectionSystem.hx", lineNumber : 44, className : "systems.SelectionSystem", methodName : "update"});
				}
			} else {
				var gui = this.entityContainer.getGuiState(e1);
				var tmp1;
	...
	,__class__: systems_SelectionSystem
};
		...
		if(child1.nodeType == Xml.Element) {
			...
			if(child1.nodeName == "properties") {
				...
				while(property.hasNext()) {
					...
					if(property1.nodeType == Xml.Element) {
						...
						if(child1.nodeName == "layer") {
							...
							if(child1.nodeName == "objectgroup") {
								...
								if(child1.nodeName == "imagelayer") {
									...
									haxe_Log.trace("unhandled child element " + child1.nodeName,{ fileName : "TiledMap.hx", lineNumber : 145, className : "tiled.TiledMap", methodName : "parseXML"});
								}
							}
						}
					}
					...
					if(element3.nodeType == Xml.Element) {
						...
						if(element3.nodeName == "properties") {
							...
							while(property2.hasNext()) {
								...
								if(property3.nodeType != Xml.Element) {
									continue;
								}
...
var tiled_TilesetImage = function(source,trans) {
	...
	if(this.texture == null) {
		haxe_Log.trace("Null this.fileName=[" + this.fileName + "] in Assets",{ fileName : "TilesetImage.hx", lineNumber : 64, className : "tiled.TilesetImage", methodName : "new"});
	}
};
...
$hxClasses["tink.testrunner.SuiteObject"] = tink_testrunner_SuiteObject;
tink_testrunner_SuiteObject.__name__ = ["tink","testrunner","SuiteObject"];
var utils_Block = $hx_exports["utils"]["Block"] = function() { };
$hxClasses["utils.Block"] = utils_Block;
utils_Block.__name__ = ["utils","Block"];
utils_Block.prototype = {
	x: null
	,y: null
	,r: null
	,__class__: utils_Block
};
var utils_Point = $hx_exports["utils"]["Point"] = function(x_,y_) {
	this.y = 0.0;
	this.x = 0.0;
	this.x = x_;
	this.y = y_;
...
var utils_EndPoint = $hx_exports["utils"]["EndPoint"] = function(x_,y_) {
	this.visualize = false;
	this.angle = 0.0;
	this.segment = null;
	this.begin = false;
	utils_Point.call(this,x_,y_);
};
$hxClasses["utils.EndPoint"] = utils_EndPoint;
utils_EndPoint.__name__ = ["utils","EndPoint"];
utils_EndPoint.__super__ = utils_Point;
utils_EndPoint.prototype = $extend(utils_Point.prototype,{
	begin: null
	,segment: null
	,angle: null
	,visualize: null
	,__class__: utils_EndPoint
});
var utils_Segment = $hx_exports["utils"]["Segment"] = function() {
};
$hxClasses["utils.Segment"] = utils_Segment;
utils_Segment.__name__ = ["utils","Segment"];
utils_Segment.prototype = {
	p1: null
	,p2: null
	,d: null
	,__class__: utils_Segment
};
var utils_Visibility = $hx_exports["utils"]["Visibility"] = function() {
	this.segments = new de_polygonal_ds_Dll();
	this.endpoints = new de_polygonal_ds_Dll();
	this.open = new de_polygonal_ds_Dll();
	this.center = new utils_Point(0.0,0.0);
	this.output = [];
	this.demo_intersectionsDetected = [];
	this.segments.toArray();
};
$hxClasses["utils.Visibility"] = utils_Visibility;
utils_Visibility.__name__ = ["utils","Visibility"];
utils_Visibility._endpoint_compare = function(a,b) {
	if(a.angle > b.angle) {
		return 1;
	}
	if(a.angle < b.angle) {
...
utils_Visibility.leftOf = function(s,p) {
	var cross = (s.p2.x - s.p1.x) * (p.y - s.p1.y) - (s.p2.y - s.p1.y) * (p.x - s.p1.x);
	return cross < 0;
};
utils_Visibility.interpolate = function(p,q,f) {
	return new utils_Point(p.x * (1 - f) + q.x * f,p.y * (1 - f) + q.y * f);
};
utils_Visibility.prototype = {
	segments: null
	,endpoints: null
	,center: null
	,open: null
	...
	,addSegment: function(x1,y1,x2,y2) {
		var segment = null;
		var p1 = new utils_EndPoint(0.0,0.0);
		p1.segment = segment;
		p1.visualize = true;
		var p2 = new utils_EndPoint(0.0,0.0);
		p2.segment = segment;
		p2.visualize = false;
		segment = new utils_Segment();
		p1.x = x1;
		p1.y = y1;
		p2.x = x2;
		p2.y = y2;
	...
	,setLightLocation: function(x,y) {
		this.center.x = x;
		this.center.y = y;
		var segment = this.segments.iterator();
		while(segment.hasNext()) {
			var segment1 = segment.next();
			var dx = 0.5 * (segment1.p1.x + segment1.p2.x) - x;
			var dy = 0.5 * (segment1.p1.y + segment1.p2.y) - y;
			segment1.d = dx * dx + dy * dy;
			segment1.p1.angle = Math.atan2(segment1.p1.y - y,segment1.p1.x - x);
			segment1.p2.angle = Math.atan2(segment1.p2.y - y,segment1.p2.x - x);
			var dAngle = segment1.p2.angle - segment1.p1.angle;
			if(dAngle <= -Math.PI) {
				dAngle += 2 * Math.PI;
			}
			if(dAngle > Math.PI) {
			...
			segment1.p1.begin = dAngle > 0.0;
			segment1.p2.begin = !segment1.p1.begin;
		}
	}
	,_segment_in_front_of: function(a,b,relativeTo) {
		var p = utils_Visibility.interpolate(b.p1,b.p2,0.01);
		var cross = (a.p2.x - a.p1.x) * (p.y - a.p1.y) - (a.p2.y - a.p1.y) * (p.x - a.p1.x);
		var A1 = cross < 0;
		var p1 = utils_Visibility.interpolate(b.p2,b.p1,0.01);
		var cross1 = (a.p2.x - a.p1.x) * (p1.y - a.p1.y) - (a.p2.y - a.p1.y) * (p1.x - a.p1.x);
		var A2 = cross1 < 0;
		var cross2 = (a.p2.x - a.p1.x) * (relativeTo.y - a.p1.y) - (a.p2.y - a.p1.y) * (relativeTo.x - a.p1.x);
		var A3 = cross2 < 0;
		var p2 = utils_Visibility.interpolate(a.p1,a.p2,0.01);
		var cross3 = (b.p2.x - b.p1.x) * (p2.y - b.p1.y) - (b.p2.y - b.p1.y) * (p2.x - b.p1.x);
		var B1 = cross3 < 0;
		var p3 = utils_Visibility.interpolate(a.p2,a.p1,0.01);
		var cross4 = (b.p2.x - b.p1.x) * (p3.y - b.p1.y) - (b.p2.y - b.p1.y) * (p3.x - b.p1.x);
		var B2 = cross4 < 0;
		var cross5 = (b.p2.x - b.p1.x) * (relativeTo.y - b.p1.y) - (b.p2.y - b.p1.y) * (relativeTo.x - b.p1.x);
		var B3 = cross5 < 0;
		if(B1 == B2 && B2 != B3) {
			return true;
		}
	...
	,sweep: function(maxAngle) {
		...
		this.output = [];
		this.demo_intersectionsDetected = [];
		this.endpoints.sort(utils_Visibility._endpoint_compare,true);
		this.open.clear();
		var beginAngle = 0.0;
		var _g = 0;
		while(_g < 2) {
			var pass = _g++;
			var p = this.endpoints.iterator();
			while(p.hasNext()) {
				var p1 = p.next();
				if(pass == 1 && p1.angle > maxAngle) {
					break;
				}
				var current_old = this.open.mSize == 0 ? null : this.open.head.val;
				if(p1.begin) {
					var node = this.open.head;
					while(node != null && this._segment_in_front_of(p1.segment,node.val,this.center)) node = node.next;
					if(node == null) {
						this.open.append(p1.segment);
					} else {
						this.open.insertBefore(node,p1.segment);
					}
				} else {
					this.open.remove(p1.segment);
				}
				var current_new = this.open.mSize == 0 ? null : this.open.head.val;
				if(current_old != current_new) {
					if(pass == 1) {
						this.addTriangle(beginAngle,p1.angle,current_old);
					}
					beginAngle = p1.angle;
				}
			}
	...
	,lineIntersection: function(p1,p2,p3,p4) {
		...
		return new utils_Point(p1.x + s * (p2.x - p1.x),p1.y + s * (p2.y - p1.y));
	}
	,addTriangle: function(angle1,angle2,segment) {
		var p1 = this.center;
		var p2 = new utils_Point(this.center.x + Math.cos(angle1),this.center.y + Math.sin(angle1));
		var p3 = new utils_Point(0.0,0.0);
		var p4 = new utils_Point(0.0,0.0);
		if(segment != null) {
			p3.x = segment.p1.x;
			p3.y = segment.p1.y;
			p4.x = segment.p2.x;
